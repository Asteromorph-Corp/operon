...
mod tests {
    ...
    #[case::simple(
        job_beta(),
        "SELECT * FROM {schema_prefix}ticket_beta WHERE status = $1;"
    )]
    fn test_get_all_ticket_query(#[case] job: JobConfig, #[case] expected: &str) {
        let stmt = GetAllTicketQuery(&job).to_string();
        assert_eq!(stmt, expected);
    }

    #[rstest]
    #[case::simple(job_beta(), "queries/ticket/get_all.rs")]
    fn test_fn_get_all(#[case] job: JobConfig, #[case] fixture_path: &str) {
        let result = fn_get_all(&job);

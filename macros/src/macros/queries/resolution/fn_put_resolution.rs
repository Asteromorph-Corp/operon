...
mod tests {
    ...
    fn test_put_resolution_query_no_dependency(
        ...
        let stmt = PutResolutionQuery(&dim).to_string();
        assert_eq!(stmt, expected);
    }

    #[rstest]
    #[case::simple(dimension_i(), "queries/resolution/put_resolution.simple.rs")]
    #[case::with_dependency(dimension_j(), "queries/resolution/put_resolution.with_dependency.rs")]
    fn test_put_resolution(#[case] dim: DimensionConfig, #[case] fixture_path: &str) {
        let result = fn_put_resolution(&dim);
